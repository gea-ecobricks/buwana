/*-----------------------------------
TEXT TRANSLATION SNIPPETS FOR signup-3.php
-----------------------------------*/
const zh_Page_Translations = {

    // confirm email
    "001-alright": "好的",
    "002-lets-confirm": "让我们确认你的邮箱。",
    "003-to-create": "为了创建你的 Buwana GoBrik 账户，我们需要确认你选择的凭证。这是我们保持联系并保护你账户的方式。点击发送按钮，我们会将激活码发送至：",
    "004-send-email-button": '📨 发送验证码 ➡',
    "006-enter-code": "请输入你的验证码：",
    "007-check-email": "请检查你的邮箱",
    "008-for-your-code": "以获取你的账户确认码。请在此输入：",
    "009-no-code": "没有收到验证码？你可以在以下时间内请求重新发送：",
    "010-email-no-longer": "你不再使用这个邮箱地址了吗？<br>如果是，请 <a href=\"signup-1.php\">创建一个新账户</a> 或联系 support@gobrik.com。",
    "011-change-email": "想要更换邮箱地址？",
    "012-go-back-new-email": "返回以输入不同的邮箱地址。",
};
